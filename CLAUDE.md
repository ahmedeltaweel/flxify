...
Flxify (flxify.dev) is a web-based developer text utility tool. It provides a syntax-highlighted code editor with 113 executable scripts accessible via a command palette (Cmd/Ctrl+B). Users paste text, run a script, and get transformed output. Think "Swiss Army knife for text transformations" — JSON formatting, Base64 encoding, hashing, case conversion, sorting, and more.
...
- **113 scripts** are auto-discovered from `scripts/*.js` and inlined as pre-compiled functions in `app.js`.
...
| File | Why it matters |
|------|----------------|
| `app.js` (generated) | Self-contained bundle — module system, all 113 scripts, BoopState, executor. **Do not edit directly; edit sources and run `node build_app.js`** |
...
| `scripts/*.js` | Source of truth for all 113 scripts |
...
| `tools/` (generated) | 113 tool pages + directory page. **Do not edit directly; generated by `node build_app.js`** |
| `sitemap.xml` (generated) | XML sitemap with all 115 URLs (homepage + directory + 113 tools). **Generated by build** |
...
| `tui/src/scripts/script-loader.js` | Loads 113 scripts at runtime for TUI. If broken, no scripts in palette |
...
13. **SEO pages are generated at build time.** `build_app.js` generates 113 static tool pages at `tools/[slug]/index.html`, a directory page at `tools/index.html`, `sitemap.xml`, and `robots.txt`. Do not edit files in `tools/` directly — they are regenerated on every build.
...
17. **`window.flxifyAutoScript` enables tool page pre-selection.** Each tool page sets `window.flxifyAutoScript = "Script Name"` before app.js loads. App.js checks this variable and logs the auto-selected script. Users can still access all 113 scripts via Cmd/Ctrl+B.
...
- **To rebuild everything:** `node build_app.js` (generates app.js + 113 tool pages + directory + sitemap + robots.txt)
...
2. **Phase 2: SEO page generation** — Reads seo-data.json + script metadata, generates:
   - `tools/[slug]/index.html` for each of 113 scripts
   - `tools/index.html` (tool directory with categories and search)
   - `sitemap.xml` (115 URLs: homepage + directory + 113 tools)
   - `robots.txt`
...
Flxify also ships as a VS Code extension at `vscode-extension/`. It's a self-contained project that shares the same 113 scripts and 7 lib modules. Currently published as v0.5.0 on the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=flxify.flxify).
...
- `tests/scripts/all-scripts.test.js` — Bulk tests: metadata validation (all 113 scripts), basic execution (106 scripts, 3 excluded), category coverage (seo-data.json cross-check)
...
Flxify also ships as a terminal-based text editor at `tui/`. It mirrors the web experience with Vim keybindings, a Command Palette, and the same 113 scripts. Launches via `node tui/bin/flxify.js` (or `flxify` when installed globally as `@flxify/cli`).
...
53. **Onboarding overlay z-index stacking context trap.** `#onboarding-overlay` and `#onboarding-box` MUST be siblings in the DOM — never parent/child. When the overlay has `position: fixed; z-index: 9000`, it creates a stacking context that traps all children. Even a child with `z-index: 9003` only ranks 9003 *within* the overlay's stacking context — it's still effectively 9000 in the root stacking context. `.onboarding-highlight` elements at `z-index: 9002` in root are ABOVE the entire overlay stacking context, covering the box and swallowing clicks. The fix: make the box a sibling `<div id="onboarding-box">` next to (not inside) `<div id="onboarding-overlay">`. Then the box's `z-index: 9003` is in the root stacking context and correctly sits above highlights (9002) and the overlay backdrop (9000). Also add `pointer-events: none` to `.onboarding-highlight` as a safety net.
...
58. **Palette Left/Right arrow keys navigate categories — intercept on the search input.** Add an `ArrowLeft`/`ArrowRight` handler inside the `searchInput` `keydown` listener. Call `e.preventDefault()` first to stop the browser from moving the text cursor inside the input. Find the current category index in `CATEGORIES`, increment/decrement with wraparound, set `activeCategory`, then call `renderPaletteCats()`, `renderCategoryBar()`, and `filterScripts()`. After `renderPaletteCats()`, scroll the newly active chip into view: `document.querySelector('#palette-cats .cat-chip.active')?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' })`. This keeps the search input focused throughout.

59. **Sidebar keyboard navigation — use a closure-local index, not a global.** Inside `initSidebar()`, declare `var sidebarActiveIdx = -1` as a closure variable. Add a `keydown` listener on `#sidebar-search`: `ArrowDown`/`ArrowUp` collect all `<a>` tags from `#sidebar-content`, increment/decrement `sidebarActiveIdx` with wraparound, add `.sidebar-item-active` to the selected item, call `scrollIntoView({ block: 'nearest' })`. `Enter` clicks the active item. `Esc` clears the input value, resets `sidebarActiveIdx = -1`, and calls `renderSidebar()` to restore the full list. Add an `input` event listener on `#sidebar-search` that also resets `sidebarActiveIdx = -1` so typing always restarts navigation from scratch.
