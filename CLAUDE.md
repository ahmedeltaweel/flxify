...
Flxify (flxify.dev) is a web-based developer text utility tool. It provides a syntax-highlighted code editor with 111 executable scripts accessible via a command palette (Cmd/Ctrl+B). Users paste text, run a script, and get transformed output. Think "Swiss Army knife for text transformations" — JSON formatting, Base64 encoding, hashing, case conversion, sorting, and more.
...
- **111 scripts** are auto-discovered from `scripts/*.js` and inlined as pre-compiled functions in `app.js`.
...
| File | Why it matters |
|------|----------------|
| `app.js` (generated) | Self-contained bundle — module system, all 111 scripts, BoopState, executor. **Do not edit directly; edit sources and run `node build_app.js`** |
...
| `scripts/*.js` | Source of truth for all 111 scripts |
...
| `tools/` (generated) | 111 tool pages + directory page. **Do not edit directly; generated by `node build_app.js`** |
| `sitemap.xml` (generated) | XML sitemap with all 113 URLs (homepage + directory + 111 tools). **Generated by build** |
...
| `tui/src/scripts/script-loader.js` | Loads 111 scripts at runtime for TUI. If broken, no scripts in palette |
...
13. **SEO pages are generated at build time.** `build_app.js` generates 111 static tool pages at `tools/[slug]/index.html`, a directory page at `tools/index.html`, `sitemap.xml`, and `robots.txt`. Do not edit files in `tools/` directly — they are regenerated on every build.
...
17. **`window.flxifyAutoScript` enables tool page pre-selection.** Each tool page sets `window.flxifyAutoScript = "Script Name"` before app.js loads. App.js checks this variable and logs the auto-selected script. Users can still access all 111 scripts via Cmd/Ctrl+B.
...
- **To rebuild everything:** `node build_app.js` (generates app.js + 111 tool pages + directory + sitemap + robots.txt)
...
2. **Phase 2: SEO page generation** — Reads seo-data.json + script metadata, generates:
   - `tools/[slug]/index.html` for each of 111 scripts
   - `tools/index.html` (tool directory with categories and search)
   - `sitemap.xml` (113 URLs: homepage + directory + 111 tools)
   - `robots.txt`
...
Flxify also ships as a VS Code extension at `vscode-extension/`. It's a self-contained project that shares the same 111 scripts and 7 lib modules. Currently published as v0.5.0 on the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=flxify.flxify).
...
- `tests/scripts/all-scripts.test.js` — Bulk tests: metadata validation (all 111 scripts), basic execution (106 scripts, 3 excluded), category coverage (seo-data.json cross-check)
...
Flxify also ships as a terminal-based text editor at `tui/`. It mirrors the web experience with Vim keybindings, a Command Palette, and the same 111 scripts. Launches via `node tui/bin/flxify.js` (or `flxify` when installed globally as `@flxify/cli`).
