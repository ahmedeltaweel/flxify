...
Flxify (flxify.dev) is a web-based developer text utility tool. It provides a syntax-highlighted code editor with 119 executable scripts accessible via a command palette (Cmd/Ctrl+B). Users paste text, run a script, and get transformed output. Think "Swiss Army knife for text transformations" — JSON formatting, Base64 encoding, hashing, case conversion, sorting, and more.
...
- **119 scripts** are auto-discovered from `scripts/*.js` and inlined as pre-compiled functions in `app.js`.
...
| File | Why it matters |
|------|----------------|
| `app.js` (generated) | Self-contained bundle — module system, all 119 scripts, BoopState, executor. **Do not edit directly; edit sources and run `node build_app.js`** |
...
| `scripts/*.js` | Source of truth for all 119 scripts |
...
| `tools/` (generated) | 119 tool pages + directory page. **Do not edit directly; generated by `node build_app.js`** |
...
2. **Phase 2: SEO page generation** — Generates:
   - `tools/[slug]/index.html` for each of 119 scripts
   - `tools/index.html` (tool directory with categories and search)
   - `sitemap.xml` (121 URLs: homepage + directory + 119 tools)
   - `robots.txt`
